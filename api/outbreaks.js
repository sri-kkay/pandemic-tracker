...
async function fetchCDCStates(){
  ...
  for(const row of rows){
    ...
    if(levelRaw === 'data unavailable' || levelRaw === 'insufficient data') continue;
...
/* The AFRO weekly bulletin is a picture-heavy 15 MB; the old 12 MB ceiling
   rejected it unread. Node on Vercel has the memory for this — the limit is
   here to stop a mis-identified 300 MB file, not to save bytes. */
const EMRO_PDF_MAX_BYTES = 32 * 1024 * 1024;
...
/* The index page's link text and its href do not always agree — a link
   labelled "Week 33" has been seen pointing at the April monthly EPR report,
   which parses to almost nothing and dates itself wrongly. So once a candidate
   is open, check the document says what it is supposed to be before believing
   anything in it. */
function looksLikeBulletin(text){
  const head = text.slice(0, 3000);
  return /weekly\s+bulletin/i.test(head)
      && /outbreaks?\s+and\s+other\s+emergencies/i.test(head);
}

/* The bulletin prints its own period on page one: "Week 33: 10 - 16 August
   2026". That beats anything inferred from a filename. */
function bulletinPeriod(text){
  const m = text.slice(0, 3000).match(
    /Week\s*(\d{1,2})\s*[:\u2013-]\s*(\d{1,2})\s*[-\u2013]\s*(\d{1,2})\s+([A-Za-z]{3,9})\.?\s+(20\d{2})/i);
  if(!m) return null;
  const mi = MONTHS.findIndex(x => x.startsWith(m[4].toLowerCase().slice(0, 3)));
  if(mi < 0) return null;
...
async function fetchAFRO(){
  ...
  for(const index of AFRO_INDEXES){
    try{
      ...
      for(const m of html.matchAll(/<a[^>]+href=["']([^"']+)["'][^>]*>([\s\S]{0,180}?)<\/a>/gi)){
        ...
        const label = cellText(m[2]);
        const hay = label + ' ' + decodeURIComponent(href);
        if(!/week/i.test(hay) && !/bulletin/i.test(hay)) continue;

        const wk = hay.match(/week[\s_#:-]*(\d{1,2})/i);
        const yr = hay.match(/(20\d{2})/);
        const entry = { href, label, week: wk ? +wk[1] : null, year: yr ? +yr[1] : null,
                        /* An index page carries monthly EPR reports and
                           annual reviews alongside the weekly bulletin. Anything
                           that calls itself a bulletin sorts above anything that
                           merely mentions a week. */
                        rank: (/bulletin/i.test(hay) ? 1e6 : 0)
                            + (yr ? +yr[1] : 0) * 100 + (wk ? +wk[1] : 0) };
  ...
  for(const candidate of pdfs.slice(0, 8)){
    try{
      ...
      if(!looksLikeBulletin(body)){
        tried.push(`${decodeURIComponent(candidate.href.split('/').pop())}: opened, but it is not the `
          + `weekly bulletin (first words: "${body.slice(0, 60).trim()}")`);
        continue;
      }
      text = body;
      newest = candidate;
      period = bulletinPeriod(body);
      break;
    }catch(err){
  ...
  /* The document's own period first, then a date range in the header, then the
     filename as a last resort — in that order of trust. */
  let end = period?.end || sitrepEndDate(text.slice(0, 1200)) || sitrepEndDate(newest.label);
  let dateVia = period ? 'the bulletin\'s own week line' : 'a date range in the header';
  if(period?.week) newest.week = period.week;
  if(!end && newest.week && newest.year){ end = isoWeekEnd(newest.year, newest.week); dateVia = 'ISO week number'; }
...
/* "fetch failed" from Vercel is not an HTTP error — it is DNS or TCP giving up
   before a response exists, which is what happens when a host refuses traffic
   from cloud IP ranges. GDELT does that intermittently.

   So the wire has more than one way in. Both providers are keyless, both are
   normalised to the same {title, domain, url, seendate} shape, and everything
   downstream — the allowlist, the corroboration rule, the no-numbers rule —
   is identical whichever one answered. The note says which was used. */
...
/* GDELT has no AND keyword — a space between two groups IS an AND, and the
   literal word "AND" makes it return an HTML error page with HTTP 200. That is
   what "non-JSON response (GDELT error page)" meant. Long queries fail the same
   way, so the disease list is split into small groups. */
function gdeltQuery(diseases){
  const terms = diseases.slice(0, 12).map(d => `"${d}"`).join(' OR ');
  return `(${terms}) (outbreak OR cases)`;
}
...
async function gdeltSearch(query, allowRetry = true){
  ...
  if(!body.trim().startsWith('{')){
    const why = body.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim().slice(0, 100);
    throw new Error('GDELT rejected the query: ' + (why || 'no explanation given'));
  }
...
/* Google News publishes search results as RSS with no key. Each item carries
   the publisher in a <source url="…"> attribute, which is what the allowlist
   needs — the <link> itself is a news.google.com redirect. */
async function googleNewsSearch(query){
  ...
  return parseRSS(xml).map((item, i) => {
    // the publisher lives in <source url="…"> on the same <item>
    const block = blocks[i] || '';
    const srcUrl = (block.match(/<source[^>]+url="([^"]+)"/i) || [])[1] || '';
    let domain = '';
    try{ domain = new URL(srcUrl).hostname; }catch(e){ domain = ''; }
    return {
      // Google appends " - Publisher" to every headline
      title: item.title.replace(/\s+-\s+[^-]{2,40}$/, '').trim(),
      domain,
      url: item.link,
      seendate: item.date ? new Date(item.date).toISOString().replace(/[-:]/g, '').slice(0, 15) + 'Z' : null
    };
  }).filter(a => a.domain && a.title);
}

const WIRE_PROVIDERS = [
  { name:'GDELT',       search: gdeltSearch },
  { name:'Google News', search: googleNewsSearch }
];
...
async function fetchWire(){
  ...
  const groups = [];
  const size = 12;               // keep each query short enough for GDELT to parse
  ...
  for(const provider of WIRE_PROVIDERS){
    ...
    if(articles.length){ usedProvider = provider.name; break; }
  }
  if(!articles.length){
...
export default async function handler(req, res){
  ...
  try{
    ...
    if(wire.disabled){
      ...
      notes.push(`Press wire via ${wire.provider}: read ${wire.read} articles, ${wire.buckets} country-disease `
